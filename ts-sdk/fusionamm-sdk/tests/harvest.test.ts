...
describe("Harvest", () => {
  ...
  const testHarvestPositionInstructions = async (poolName: string, positionName: string) => {
    const [mintAName, mintBName] = poolName.split("-");
    ...
    const { instructions: harvest_instructions, feesQuote } = await harvestPositionInstructions(
      rpc,
      positionMintAddress,
    );
    ...
    assert.strictEqual(feesQuote.feeOwedA, tokenAAfter.data.amount - tokenABefore.data.amount);

    assert.strictEqual(feesQuote.feeOwedB, tokenBAfter.data.amount - tokenBBefore.data.amount);
  };

  const testHarvestPositionInstructionsWithoutFees = async (_poolName: string, positionName: string) => {
    const positionMintAddress = positions.get(positionName)!;
  ...
  for (const poolName of poolTypes.keys()) {
    for (const positionTypeName of positionTypes.keys()) {
      ...
      it(`Should harvest a position without fees for ${positionNameTE}`, async () => {
        await testHarvestPositionInstructionsWithoutFees(poolName, positionNameTE);
      });
    }
  }
});
