...
describe("Close Position", () => {
  ...
  const testClosePositionInstructions = async (poolName: string, positionName: string) => {
    const [mintAName, mintBName] = poolName.split("-");
    ...
    assert.strictEqual(quote.tokenEstA + feesQuote.feeOwedA, tokenAAfter.data.amount - tokenABefore.data.amount);

    assert.strictEqual(quote.tokenEstB + feesQuote.feeOwedB, tokenBAfter.data.amount - tokenBBefore.data.amount);
  };
  ...
  it("Should close a position without liquidity", async () => {
    ...
    await assert.doesNotReject(testClosePositionInstructions(poolName, positionName));
  });

  it("Should throw an error if the position mint can not be found", async () => {
    const positionMintAddress: Address = address("123456789abcdefghijkmnopqrstuvwxABCDEFGHJKL");
